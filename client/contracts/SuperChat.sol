...
contract SuperChat {
    ...
    function SChat(address  _recipient, uint256  duration)
        public
        payable
    {
        sender = msg.sender;
    ...
    function isValidSignature(uint256 amount, bytes memory signature)  
        ...
        returns (bool)
    {
        bytes32 message = prefixed(keccak256(abi.encodePacked(this , amount))); //changes
    ...
    function close(uint256 amount, bytes memory signature ) public payable {
        require(msg.sender == recipient);
        require(isValidSignature(amount, signature));

        payable(recipient).transfer(amount);
        selfdestruct(payable(sender));  //changes
    }
